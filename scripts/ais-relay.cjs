...
const API_KEY = (process.env.AISSTREAM_API_KEY || process.env.VITE_AISSTREAM_API_KEY || '').trim();
...
// OpenSky Credentials (supports both OAuth2 and Basic Auth fallback)
...
const RSS_CACHE_TTL_MS = 20 * 60 * 1000; // 20 min — alignment for v0.2.2-alpha
...
async function getOpenSkyToken() {
  ...
  // Prioritize Basic Auth if username/password are provided and client ID seems to be an email/username (standard for users)
  // Or if OAuth2 info is totally missing.
  const hasBasicAuth = !!(username && password);
  const hasOAuth2 = !!(clientId && clientSecret && clientId !== username);

  if (hasBasicAuth && (!hasOAuth2 || openskyAuthCooldownUntil > Date.now())) {
    // Return Basic Auth string as the "token"
  ...
  return new Promise((resolve) => {
    const request = https.get(url, {
      ...
      headers: {
        ...
        'User-Agent': 'WorldMonitor/1.0',
        'Authorization': (token && token.startsWith('Basic ')) ? token : (token ? `Bearer ${token}` : undefined),
      },
      timeout: 15000,
    }, (response) => {
      let data = '';
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/health' || pathname === '/') {
    ...
    diag.steps.push({ step: 'env_check', hasClientId: !!clientId, hasClientSecret: !!clientSecret, hasUsername: !!username, hasPassword: !!password });
    diag.steps.push({
      step: 'auth_state',
      ...
      if (rssCached) {
        ...
        if (Date.now() - rssCached.timestamp < ttl) {
          return sendCompressed(req, res, rssCached.statusCode || 200, {
            ...
            'Cache-Control': rssCached.statusCode >= 200 && rssCached.statusCode < 300 ? 'public, max-age=600' : 'no-cache',
            'CDN-Cache-Control': rssCached.statusCode >= 200 && rssCached.statusCode < 300 ? 'public, max-age=1200, stale-while-revalidate=600' : 'no-store',
            'X-Cache': 'HIT',
          }, rssCached.data);
        }
      }
          ...
          const request = protocol.get(url, {
            ...
            stream.on('end', () => {
              ...
              sendCompressed(req, res, response.statusCode, {
                ...
                'Cache-Control': response.statusCode >= 200 && response.statusCode < 300 ? 'public, max-age=600' : 'no-cache',
                'CDN-Cache-Control': response.statusCode >= 200 && response.statusCode < 300 ? 'public, max-age=1200, stale-while-revalidate=600' : 'no-store',
                'X-Cache': 'MISS',
              }, data);
            });
            stream.on('error', (err) => {
