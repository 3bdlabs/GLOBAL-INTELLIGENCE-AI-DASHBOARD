...
export class PanelLayoutManager implements AppModule {
  ...
  renderLayout(): void {
    this.ctx.container.innerHTML = `
      <div class="header" id="header-mount"></div>
    ...
    if (headerMount) {
      render(h(Header, { isDesktopApp: Boolean(this.ctx.isDesktopApp) }), headerMount);
    }
  ...
  private createPanels(): void {
    ...
    if (SITE_VARIANT !== 'happy') {
      ...
      if (this.shouldCreatePanel('events')) {
        this.ctx.panels['events'] = new TechEventsPanel('events', () => this.ctx.allNews);
      }

      if (this.shouldCreatePanel('service-status')) {
    ...
    // Intelligence / Master Analyst panels (lazy-loaded to avoid circular dependency)
    this.lazyPanel('intelligence-panel', () =>
      import('@/components/IntelligencePanelWrapper').then(m => new m.IntelligencePanelWrapper()),
    );
    this.lazyPanel('master-analyst', () =>
      import('@/components/MasterAnalystPanelWrapper').then(m => new m.MasterAnalystPanelWrapper()),
    );
